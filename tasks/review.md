...
> **Status 2026-07-10: all items R1–R12 resolved in session.** R1: new figure `Figures/fitness_over_generations.png` (generated by `analysis/training_curves_pop200.py` from mlflow.db, 30 seeds × 3 conditions, 95% CI bands) added to §5.2 with a five-sentence dynamics paragraph — curves never separate by more than 0.03 normalized units. R2: a literature/research-question paragraph added to the Discussion (Hagg et al. static-data results, parsimony direction reattributed to the mutation machinery, "personalities" hypothesis verdict, degeneration caveat). R3 "pre-specified"; R4 wall-clock sentence cut and recap trimmed from 11 to 7 sentences; R5 reworded around seed count; R6 hedged hop-in-place sentence added to §5.4 (episodes not rendered — verification left open); R7 stray label removed, refs point to `sec:main-study`; R8 all three minipage figures converted to `subcaption` subfigures (`\usepackage{subcaption}` added); R9 legacy block (172 lines, incl. the last [PLACEHOLDER]s) deleted; R10 retitled "Discussion"; R11 raw-scale sentence added below the pairwise stats; R12 follow-ups claim narrowed to limitations 1, 3, and 5. Humanizer pass run over all new prose (F2 sweep clean).
