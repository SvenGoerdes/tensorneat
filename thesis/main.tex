...
We evaluated NEAT and HA-NEAT on a single multi-task control problem: one evolved network must control both the Hopper body (11 observations, 3 actions) and the Walker2D body (17 observations, 6 actions) using the same weights.
The network was sized to the larger task; Hopper observations were zero-padded to 17 dimensions, and the 6-dimensional output was sliced to 3 for Hopper's actuators (Section~\ref{subsec:network-architecture}).
...
Each condition was evolved at population 200 for 750 generations across 30 independent random seeds, giving 90 runs in total, all on the MJX (MuJoCo on GPU) physics backend.

The selection signal was the normalized minimum of the two per-task scores, $\mathrm{fit} = \min(r_{\text{hopper}}/3000,\, r_{\text{walker2d}}/5000)$.
Because fitness tracks the weaker task, a genome cannot raise its score by specializing in one body and neglecting the other.
Section~\ref{subsec:fitness-evaluation} gives the conceptual rationale for this objective, and the calibration study (Section~\ref{subsec:calibration}) supplies its empirical justification against the sum and product alternatives.
The same calibration study fixed the 750-generation budget: it located the diminishing-returns regime and confirmed that the budget transfers to the population-200 setting.
Smaller-scale preliminary runs (populations of a few thousand at two to three seeds) shaped this design, including motivating the addition of the ablation condition, and were superseded by the larger-sample main study reported here.
...
A single run took a median of roughly 3.4~h (mean 3.8~h) on one GPU, and the full 90-run campaign consumed about 340 GPU-hours; Section~\ref{subsec:compute-resources} lists the hardware.
...
% \label{subsec:ablation-activation-diversity-vs-ha-neat}
...
% \label{subsubsec:training-performance}
% 
% \begin{table}[h]
% \centering
...
% \label{subsubsec:statistical-comparison}
% 
...
% \label{subsec:multi-task-performance-comparison}
% 
...
Because the three conditions reached statistically indistinguishable fitness (Section~\ref{res:main-fitness}), we examined whether they arrived there with equivalent genome structure.
...
\subsection{Discussion of the main-study analyses}
