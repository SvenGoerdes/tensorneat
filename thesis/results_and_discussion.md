...
We trained NEAT and HA-NEAT on the same multi-task problem: a single evolved network must control both a Hopper (11 observations, 3 actions) and a Walker2D (17 observations, 6 actions). The network is sized to the larger task. Hopper observations are zero-padded to 17 dimensions, and the 6-dimensional output is sliced to 3 for Hopper's actuators. Fitness is the normalized minimum across both tasks, that is, the worse of the two normalized per-task rewards. A network that learns Hopper well but neglects Walker2D scores poorly. The choice of normalized-minimum aggregation and its justification are described in Section 3 (Methodology).
...
HA-NEAT changes two things relative to standard NEAT. First, it draws per-node activations from a set of five functions (tanh, sigmoid, relu, sin, identity) instead of a single fixed one. Second, it uses OriginConn, a connection gene that tracks historical innovation markers. When a node's activation is mutated, all connections touching that node receive new markers, which increases the genetic distance between the mutant and its parent. In practice, this pushes the mutant into a separate species, shielding it from crossover with genomes that still use the old activation.

The performance difference in Section 5.2 could come from either change, or from their interaction. To isolate the two, we ran an ablation that keeps the full HA-NEAT machinery (OriginConn, HANEATMutation, marker reassignment) but restricts it to a single activation function (tanh). The activation mutation fires as usual, but since there is only one option, nothing actually changes. This gives us HA-NEAT's structural changes without the diversity.

**Table 3. Ablation results: aggregate normalized-minimum fitness.**
...
<!-- Figure 6: Ablation bar chart.
     Three grouped bars: NEAT (blue), HA-NEAT full (orange), HA-NEAT ablation (green).
     Use mean +/- std across seeds.
     Source: results/multi_task_neat_vs_haneat_ablation/ + v3 HA-NEAT as "full" reference.
     NOTE: scale difference — ablation used pop=1024 vs v3's pop=2048. Mention in caption. -->

The ablation averaged 0.364 across three seeds, slightly below the NEAT baseline of 0.379 at the same configuration. With diversity removed, the OriginConn machinery and marker reassignment on their own did not improve performance. If anything, they introduced a small overhead without a corresponding benefit. This points to activation diversity as the operative ingredient in HA-NEAT's advantage, not the speciation changes that come with it. [PLACEHOLDER: include full HA-NEAT bar from final experiment data for direct comparison. Note the population size mismatch (ablation: 1024, main comparison: 2048+) when interpreting the magnitude of differences.]
